"""Router node for FitBuddy Agent."""
...
from ..models import ContextPacket, FitBuddyState
...
async def router_node(state: FitBuddyState, config: RunnableConfig) -> FitBuddyState:
    """
    ...
    if state.get("finished", False) and state.get("final_advice"):
        if msgs and isinstance(msgs[-1], HumanMessage):
            logger.info("[ROUTER] Final advice generated but user sent new message - resetting finished flag")
    ...
    logger.debug(
        f"[ROUTER] Section: {current_section.value if current_section else 'unknown'}, "
        f"Directive: {directive}, Finished: {state.get('finished', False)}"
    )
